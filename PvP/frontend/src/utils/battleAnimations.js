...
export const animateAttack = (
  ...
  try {
    ...
    setTimeout(() => {
      ...
      console.log('About to show damage number with value:', damage, 'isBlocked:', isBlocked);
...
/**
 * Enhanced damage number display with FIXED logic and validation
 */
export const showDamageNumber = (
  ...
  try {
    // VALIDATION: Ensure amount is a number - handle various input types
    let damageAmount = 0;
    ...
    if (typeof amount === 'number') {
      ...
      // Handle case where damage might be nested in an object
    ...
    console.log('showDamageNumber called with:', {
      amount: amount,
    ...
    // FIXED LOGIC: Show damage numbers based on actual damage value
    if (damageAmount === 0) {
      // Only show BLOCKED/MISS when damage is actually 0
      if (isBlocked) {
      ...
      // Add critical class if it's a critical hit
      if (isCritical) {
        className += ` ${ANIMATION_CLASSES.DAMAGE_TEXT.CRITICAL}`;
      }
    }
    
    // Force log to ensure we're showing damage when we should
    if (damageAmount > 0) {
      console.log(`SHOWING DAMAGE: ${displayText} (${damageAmount} damage)`);
    }
    ...
    if (isCritical && damageAmount > 0) {
      ...
      // Remove after animation - DOUBLED DURATION
      setTimeout(() => {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // ALWAYS show damage/heal number with validation
        showDamageNumber(
          targetElement, 
          spellDamage, 
          'magical', 
